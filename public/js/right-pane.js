...
(function () {
  ...
  function initRightPane() {
    ...
    const slug = page?.getAttribute('data-slug') || null;
    if (slug !== currentSlug) {
      graphLoaded = false;
      currentSlug = slug;
    }
    if (pane.dataset.rpBound === '1') return;
    pane.dataset.rpBound = '1';
  ...
  document.addEventListener('DOMContentLoaded', initRightPane);
  window.addEventListener('wipa:split-built', initRightPane);
  window.addEventListener('wipa:navigate', () => setTimeout(initRightPane, 60));
})();
