...
- `/api/chat` RAG uses three-source retrieval: (R) Readwise vector index (`Raw/readwise-vector-index.json`, cosine ≥ 0.78); (W-vec) Wiki/Notes/Projects vector index (`Raw/wiki-vector-index.json`, cosine ≥ 0.65, fills first); (W-kw) MiniSearch keyword search fills remaining W slots. Both indices share one `Xenova/multilingual-e5-small` extractor in `src/search/vectorStore.js`.
- Wiki vector index is built by `scripts/embed-wiki.js` (chunks .md files ~800 chars with 100-char overlap, mtime-based `--incremental`). Run once after setup; not yet in daily cron (planned P1).
...
- Chat retrieval uses only the latest user message as the search query — no conversation-aware rewrite for pronouns or follow-ups (planned P1 work).
